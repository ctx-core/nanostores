...
test('be_computed_pair_|-autosubscribe|+id|+ns', ()=>{
	...
	equal(foobar_(ctx), 2)
	equal((ctx.s.test_ns.get('foobar') as ReadableAtom_<number>).$, 2)
	base__set(ctx, 2)
	...
	equal(foobar_(ctx), 3)
	equal((ctx.s.test_ns.get('foobar') as ReadableAtom_<number>).$, 3)
})
...
test('be_computed_pair_|be', ()=>{
	...
	equal(foobar$_(ctx)._, 2)
	equal(foobar_(ctx), 2)
	equal((ctx.s.test_ns.get('foobar') as ReadableAtom_<number>)._, 2)
	...
	equal(foobar$_(ctx)._, 3)
	equal(foobar_(ctx), 3)
	equal((ctx.s.test_ns.get('foobar') as ReadableAtom_<number>)._, 3)
	equal(foobar$_(ctx).custom, 'custom-val')
})
test.run()
